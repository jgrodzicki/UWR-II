...
def load_ngrams_and_create_mapping(filename, n):
...
if __name__ == '__main__':
    successor_mapping_bigram = load_ngrams_and_create_mapping(filename='data/poleval_2grams.txt', n=2)
    sentences = create_sentences_for_bigram(successor_mapping=successor_mapping_bigram, length=200)
    print('From bigrams')
    print('\n'.join(sentences))
    open('results/task3_bigram.txt', 'w+').writelines('\n'.join(sentences))
    
    print('-'*100)
    
    successor_mapping_trigram = load_ngrams_and_create_mapping(filename='data/poleval_3grams.txt', n=3)
    sentences = create_sentences_for_trigram(successor_mapping=successor_mapping_trigram, length=200)
    print('From trigrams')
    print('\n'.join(sentences))
    open('results/task3_trigram.txt', 'w+').writelines('\n'.join(sentences))
