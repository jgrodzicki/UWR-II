...
class ES:
    def __init__(self, domain, dims, cost_func, tau, tau0, population_size=1000, offspring_size=500, parent_choice_method='roulette', replacement_method='mulambda', init_domain=None, max_iters=1000):
        ...
        if type(self.init_domain) is tuple:
            self.init_domain = np.array([init_domain] * dims)
        if self.init_domain is None:
            self.init_domain=self.domain
    ...
    def _random_ind(self):
        x = np.random.uniform(self.init_domain[:,0], self.init_domain[:,1], size=self.dims)
