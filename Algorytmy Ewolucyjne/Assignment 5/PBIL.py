...
class PBIL:
    ...
    def fit(self, new_max_iter=None):
        if new_max_iter is None:  # first fitting
            ...
            if self.with_tqdm:
                range_ = trange(self.max_iter, desc='PBIL', position=0, leave=True)
            else:
    ...
    def plot_evals(self, with_params=False):
        plt.figure()
        if with_params:
            plt.title(f'PBIL eval changes for learning_rate: {self.learning_rate}, \
mutation_prob: {self.mutation_prob}, disturbance_rate: {self.disturbance_rate}')
        else:
            plt.title('PBIL eval changes')
        
        plt.plot(range(self.max_iter), self.best_evals, c='g', linewidth=0.5, label='Max')
        plt.plot(range(self.max_iter), self.mean_evals, c='b', linewidth=0.5, label='Mean')
        plt.plot(range(self.max_iter), self.worst_evals, c='r', linewidth=0.5, label='Min')
        plt.legend()
        plt.show()
