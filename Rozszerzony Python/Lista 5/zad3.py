...
def opt_elem(row_index):
    def count_not_good(arr, sp):  # count minimal not good elements for each row
    ...
    min = 2 * len(monogram[row_index]) + 1
    good_ind = -1
    for i in range(len(monogram[row_index])):
        change(row_index, i)
        x = count_not_good(monogram[row_index], spec[row_index]) + count_not_good(monogram[rows + i], spec[rows + i])
        if x < min:
            min = x
            good_ind = i
